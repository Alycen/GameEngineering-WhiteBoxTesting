...
void Camera::setViewPosition(sf::Vector2f pos) 
{
	...
	else if (pos.x < window_width / 3.2)
		camView.setCenter(window_width / 3.2, camView.getCenter().y);
	else if (pos.x > 4800)
		camView.setCenter(4800, camView.getCenter().y);

	if (pos.y >= window_height / 3 && pos.y <= 5050)
		camView.setCenter(camView.getCenter().x, pos.y);
	else if (pos.y < window_height / 3)
		camView.setCenter(camView.getCenter().x, window_height / 3);
	else if (pos.y > 5050)
		camView.setCenter(camView.getCenter().x, 5050);
}